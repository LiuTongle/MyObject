...
public class MessageDaoImpl extends GenericDaoHibernate<Message, Long> implements IMessageDao {
    ...
    @Override
    public List<Message> getMessagesByUserIdAndType(Long id, Integer type, Integer userType, Integer start, Integer offset, List<Long> receivedIds) {
        DetachedCriteria query = DetachedCriteria.forClass(Message.class);
        Criterion criterion = Restrictions.eq("messageUserId", id);
        Criterion criterion1 = Restrictions.eq("messageType", type);
        Criterion criterion2 = Restrictions.eq("messageUserType", userType);
        query.add(criterion);
