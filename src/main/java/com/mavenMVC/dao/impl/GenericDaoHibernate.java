...
public class GenericDaoHibernate<T, PK extends Serializable> implements GenericDao<T, PK> {
	...
	@Override
	public List<T> findPageByDetachedCriteria(final DetachedCriteria criteria, final int from, final int size) {
		return (List<T>) getHibernateTemplate().executeWithNativeSession(new HibernateCallback() {
			public Object doInHibernate(Session session) throws HibernateException, SQLException {
				Criteria criteria1 = criteria.getExecutableCriteria(session);
				if (from >= 0 && size > 0)
					return criteria1.setFirstResult(from).setMaxResults(size).list();
				else
					return criteria1.list();
			}
		});
