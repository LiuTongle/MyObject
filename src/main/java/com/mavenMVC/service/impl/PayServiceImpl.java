...
public class PayServiceImpl implements IPayService {
	...
	public Charge payOrder(Long orderId, Integer money, String channel, String ip) {
		...
		chargeParams.put("subject", "商品費用subject");
		chargeParams.put("body", "商品費用body");
		try {
	...
	public Refund refundOrder(Order order) {
		...
		if ((order.getOrderProceedTime() < 14 * 60) && (order.getOrderType() == 0)) {
			refundMoney = new Double(
					(double) doctor.getDoctorPrice() * (1 - (double) order.getOrderProceedTime() / (15 * 60)))
							.intValue();
			// refundMoney = doctor.getDoctorPrice() - new
			// Double(money).intValue();
			Pingpp.apiKey = Code.PING_CODE;
			Pingpp.privateKey = Code.PING_PRIVATE_KEY;
	...
	@Override
	public String transferOrder(Doctor doctor, Integer amount, String channel, String bankNo, String bankUserName,
			String openBank, String bankProv, String bankCity, String wxUserOpenId, String wxUserName) {
		try {
	...
	// "/Users/lizai/Desktop/RSACert/rsa_private_key.pem";
